...
echo Implementation fix: rasterize all D2A owner fields before extracting candidate masks.
...
py -3 -m unittest tests.test_akerpuls_d2b_full_skane_true_loo_fusion_v1 tests.test_akerpuls_d2b_owner_raster_fix_v1 -v
if errorlevel 1 goto :fail

echo.
echo D2B starts now. This is the expensive local TRUE-LOO stage: about 50,704 omission refits.
echo The prior candidate-only cell caches are intentionally invalidated by the implementation-fix SHA.
echo New completed cell TRUE-LOO outputs are hash-verified and reused after interruption.
echo.
py -3 -u src\156_akerpuls_d2b_owner_raster_fix_v1.py --output-dir "%OUT%"
set "RC=%ERRORLEVEL%"
