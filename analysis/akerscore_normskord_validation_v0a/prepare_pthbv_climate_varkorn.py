#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Prepare 2011–2025 Apr–Jul PTHBV climate covariates for spring-barley field-years."""
...
from run_varkorn_validation import prepare_crop, CROP_LABEL
...
def main() -> int:
    ...
    try:
        ...
        if not _same_month_axis(ds_t[t_time].values, ds_p[p_time].values):
            raise RuntimeError(
                "tas/pr calendar-month axes differ: "
                f"tas={_month_keys(ds_t[t_time].values)[:3]}..., "
        ...
        qa = {
            "version": "akerscore-normskord-pthbv-varkorn-v0a",
            "crop": CROP_LABEL,
            "period": {"start_year": START_YEAR, "end_year": END_YEAR, "months": list(MONTHS)},
            "spatial_method": "exact current-field polygon x PTHBV grid-cell intersection; then spring-barley field-year area weighting",
            "crop_qa": crop_qa,
        ...
        print("\nVÅRKORN PTHBV CLIMATE PREPARATION: PASS")
        return 0
    finally:
