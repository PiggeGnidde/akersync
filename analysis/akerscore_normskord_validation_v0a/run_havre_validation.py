#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Replicate the ÅkerScore ↔ normskörd bridge for oats (Havre)."""
...
def prepare_crop(ctx: pd.DataFrame, hist: pd.DataFrame, score: pd.DataFrame):
    ...
    bad = labels[~labels["dominant_crop_name"].fillna("").str.lower().str.contains("havre")]
    if len(bad):
...
def main() -> int:
    ...
    if len(fit_df) < 8:
        raise RuntimeError(f"Too few SKO with Havre norm yield and crop data: {len(fit_df)}")
    ...
    result = {
        "version": "akerscore-normskord-havre-validation-v0a",
        ...
        "monotone": mono,
        "guardrail": "Independent crop replication; official PXWeb target fetched at run time; no Hasund calibration used in fitting.",
    }
    (out / "results.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")

    print("="*88); print("ÅkerScore ↔ Havre normskörd 2026 — replication v0a"); print("="*88)
    ...
    for s, yy in zip(KNOTS, knot_y): print(f"    ÅkerScore {s:5.1f} -> {yy:5.3f} t/ha")
    print("\nSKO TABLE")
