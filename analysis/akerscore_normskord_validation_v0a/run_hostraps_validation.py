#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Replicate the ÅkerScore ↔ official 2026 normskörd bridge for Höstraps."""
...
def prepare_crop(ctx: pd.DataFrame, hist: pd.DataFrame, score: pd.DataFrame):
    ...
    labels = (
        ...
        .drop_duplicates().sort_values(["history_year", "dominant_crop_name"])
    )
    # Historical dictionaries can vary in exact wording, but code 20 must remain rape/rapeseed.
    bad = labels[~labels["dominant_crop_name"].fillna("").str.lower().str.contains("raps")]
    if len(bad):
        raise RuntimeError(
            f"Crop code {CROP_CODE} has unexpected historical labels:\n{bad.to_string(index=False)}"
        )
...
def main() -> int:
    ...
    if len(fit_df) < 8:
        raise RuntimeError(f"Too few SKO with Höstraps norm yield and crop data: {len(fit_df)}")
    ...
    result = {
        "version": "akerscore-normskord-hostraps-validation-v0a",
        "crop_code": CROP_CODE, "crop": CROP_LABEL, "target_year": 2026,
        "n_sko": int(len(fit_df)), "sko_ids": fit_df["sko_id"].tolist(),
        "crop_qa": crop_qa,
        "linear": {
            ...
            "slope_ci95": linear["slope_ci95"],
            "r2": linear["r2"], "rmse_t_ha": linear["rmse_t_ha"],
            "mae_t_ha": linear["mae_t_ha"], **loo,
        },
        "monotone": mono,
        "guardrail": (
            "Independent crop replication. Crop code 20 is winter rapeseed; official PXWeb target "
            "is fetched at run time. No Hasund or previous crop coefficient is used in fitting."
        ),
    }
    (out / "results.json").write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding="utf-8")

    print("="*88)
    print("ÅkerScore ↔ Höstraps normskörd 2026 — replication v0a")
    print("="*88)
    ...
    print("\nSKO TABLE")
    cols = ["sko_id", "norm_t_ha", "n_companies", "unique_fields", "field_years",
            "mean_akerscore_areaweighted", "linear_residual_t_ha"]
    print(fit_df[cols].to_string(index=False))
    print("\nHÖSTRAPS VALIDATION: PASS")
    return 0
