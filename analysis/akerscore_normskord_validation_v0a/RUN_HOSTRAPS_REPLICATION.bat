...
set "OUT=%ROOT%\work\akerscore_normskord_hostraps_v0a"
set "HERE=%~dp0"
set "FETCH=%HERE%fetch_normskord_hostraps_2026.py"
set "BASE=%HERE%run_hostraps_validation.py"
set "PREP=%HERE%prepare_pthbv_climate_hostraps.py"
set "MODEL=%HERE%run_climate_validation.py"
set "NORM=%OUT%\normskord_hostraps_2026.csv"
...
echo ====================================================================================
echo Hostraps independent replication: AkerScore + Normskord + PTHBV climate
echo ====================================================================================
echo.
echo [1/5] Fetch official Jordbruksverket 2026 Hostraps norm yields...
%PY% "%FETCH%" --output "%NORM%"
if errorlevel 1 goto :fail

echo.
echo [2/5] Hostraps normskord bridge...
%PY% "%BASE%" --input-dir "%INPUT%" --output-dir "%OUT%\base" --norm-csv "%NORM%"
if errorlevel 1 goto :fail

echo.
echo [3/5] Hostraps-field weighted PTHBV climate...
%PY% "%PREP%" --temp-netcdf "%TAS%" --precip-netcdf "%PR%" --input-dir "%INPUT%" --local-paths "%LOCALPATHS%" --output-dir "%OUT%\climate"
if errorlevel 1 goto :fail

echo.
echo [4/5] All published Skane-domain Hostraps SKO...
...
echo.
echo ====================================================================================
echo RUN_HOSTRAPS_REPLICATION: PASS
echo ====================================================================================
echo Main results:
echo   %NORM%
...
echo.
echo ====================================================================================
echo RUN_HOSTRAPS_REPLICATION: FAIL
echo ====================================================================================
exit /b 1
