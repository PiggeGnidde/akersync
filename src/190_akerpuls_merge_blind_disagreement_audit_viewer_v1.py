...
Predeclared sample among the exact 22,358 M0-assessable pairs:
  HH: M0 upper decile, M1 p_samecrop upper decile
  HL: M0 upper decile, M1 p_samecrop lower decile
  LH: M0 lower decile, M1 p_samecrop upper decile
  LL: M0 lower decile, M1 p_samecrop lower decile
...
STATUS = "PASS_TO_BLIND_MERGE_DISAGREEMENT_AUDIT"
...
HIGH_Q = 0.90
LOW_Q = 0.10
SAMPLE_SALT = "akerpuls-merge-disagreement-audit-sample-v1|2026-09-20|54a567c2"
BLIND_ORDER_SALT = "akerpuls-merge-disagreement-audit-order-v1|2026-09-20|54a567c2"
BUFFER_M = 100.0
PANEL_PX = 380
HEADER_PX = 28
TITLE_PX = 44
...
def main() -> int:
    ...
    print("AKERPULS MERGE BLIND DISAGREEMENT AUDIT VIEWER V1")
    print(f"GIT_HEAD={head}")
    print(f"M2_DIAGNOSTIC_FREEZE_SHA256={EXPECTED_M2_FREEZE_SHA256}")
    print("PROGRESS=BUILD_PREDECLARED_4X25_SAMPLE")
    ...
    strata_masks = {
        "HH": (a["satellite_score_percentile"] > HIGH_Q) & (a["m1_pct"] > HIGH_Q),
        "HL": (a["satellite_score_percentile"] > HIGH_Q) & (a["m1_pct"] <= LOW_Q),
        "LH": (a["satellite_score_percentile"] <= LOW_Q) & (a["m1_pct"] > HIGH_Q),
        "LL": (a["satellite_score_percentile"] <= LOW_Q) & (a["m1_pct"] <= LOW_Q),
    }
    ...
    manifest = {
        "schema_version": "akerpuls-merge-blind-disagreement-audit-viewer-v1",
        "status": STATUS,
        ...
        "sample": {
            ...
            "strata": {
                "HH": "M0 percentile > 0.90 AND M1 percentile > 0.90",
                "HL": "M0 percentile > 0.90 AND M1 percentile <= 0.10",
                "LH": "M0 percentile <= 0.10 AND M1 percentile > 0.90",
                "LL": "M0 percentile <= 0.10 AND M1 percentile <= 0.10",
            },
            "sample_method": "LOWEST_SHA256_25_PER_STRATUM_WITH_FROZEN_SALT",
