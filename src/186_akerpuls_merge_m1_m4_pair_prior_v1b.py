...
STATUS = "PASS_TO_M1_M4_PAIR_PRIOR_V1B_FREEZE_REVIEW"
...
def main() -> int:
    ...
    print("AKERPULS MERGE M1 M4 PAIR PRIOR V1B ID-BRIDGE")
    ...
    # Explicit frozen namespace bridge:
    # D2A/M0 infer_ids: 2025|BLOCKID|SKIFTESBETECKNING
    # ÅkerMinne/M4 current_field_id: BLOCKID|SKIFTESBETECKNING
    def to_m4_id(s: pd.Series) -> pd.Series:
        ss = s.astype("string")
        if not bool(ss.str.startswith("2025|").all()):
            bad = ss[~ss.str.startswith("2025|")].head(5).tolist()
            raise RuntimeError(f"Unexpected M0 field-ID namespace; expected 2025| prefix. Examples={bad}")
        out = ss.str.slice(start=5)
        if bool(out.eq("").any()) or bool(out.isna().any()):
            raise RuntimeError("Empty/null canonical M4 ID after stripping frozen 2025| prefix")
        return out

    field_a_m4 = to_m4_id(pairs["field_a"])
    field_b_m4 = to_m4_id(pairs["field_b"])
    if field_a_m4.nunique() > pairs["field_a"].nunique() or field_b_m4.nunique() > pairs["field_b"].nunique():
        raise RuntimeError("Impossible namespace-bridge cardinality expansion")
    bridge_pairs = field_a_m4.astype(str) + "||" + field_b_m4.astype(str)
    if bridge_pairs.duplicated().any():
        raise RuntimeError("Namespace bridge creates duplicate canonical pair keys")

    print("ID_NAMESPACE_BRIDGE=M0:2025|BLOCKID|SKIFTESBETECKNING -> M4:BLOCKID|SKIFTESBETECKNING")
    ...
    missing_a = ~field_a_m4.isin(pmat.index)
    missing_b = ~field_b_m4.isin(pmat.index)
    if bool(missing_a.any()) or bool(missing_b.any()):
        raise RuntimeError(f"M1 V1B canonical-ID join missing fields: A={int(missing_a.sum())} B={int(missing_b.sum())}")

    a = pmat.loc[field_a_m4.to_numpy()].reset_index(drop=True)
    b = pmat.loc[field_b_m4.to_numpy()].reset_index(drop=True)
    ...
    result = pd.DataFrame({
        ...
        "field_a_m0": pairs["field_a"].to_numpy(),
        "field_b_m0": pairs["field_b"].to_numpy(),
        "field_a_m4": field_a_m4.to_numpy(),
        "field_b_m4": field_b_m4.to_numpy(),
    ...
    summary = {
        ...
        "pair_universe": {
            ...
            "id_namespace_bridge": "strip exact leading 2025| from frozen M0/D2A IDs to match ÅkerMinne/M4 current_field_id",
    ...
    print("ID_NAMESPACE_BRIDGE_MATCH_A=27146/27146 ID_NAMESPACE_BRIDGE_MATCH_B=27146/27146")
