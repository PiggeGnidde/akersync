...
function aknHelp(){return`<details class="akn-help"><summary>Begrepp och gula markeringar</summary><div class="akn-help-body"><div class="akn-help-item"><div class="akn-help-term">Skiftesjustering</div>Skillnaden mellan skiftets ÅkerNorm och den officiella SKO-normen. Justeringen beror på hur skiftets ÅkerScore avviker från referenspoängen för samma gröda och SKO.</div><div class="akn-help-item"><span class="akn-help-chip">Högre osäkerhet</span><div>Havremodellen är mer osäker, eller skiftets ÅkerScore ligger utanför det centrala referensintervallet (P05–P95) för grödan och SKO. Värdet klipps inte och bör tolkas försiktigare.</div></div><div class="akn-help-item"><span class="akn-help-chip">Svag skiftesjustering</span><div>För höstraps är den validerade ÅkerScore-effekten liten: 10 score-poäng ändrar ÅkerNorm med 0,05 t/ha. Värdet ligger därför nära den officiella SKO-normen.</div></div><div class="akn-help-item"><span class="akn-help-chip">Årsberoende grödkod</span><div>Samma numeriska grödkod har olika officiell benämning olika år. Öppna ”Så beräknas och bedöms värdet” för den årsvisa listan.</div></div><div class="akn-help-item"><span class="akn-help-chip">Flera grödor</span><div>I ÅkerMinne betyder detta att den näst största grödan täcker minst 5 % av dagens skifte under året. Andelarna visas under huvudgrödan.</div></div></div></details>`}
...
function akernormToggle(element){if(!element.open||element.dataset.loaded==="1")return;const body=element.querySelector(".akn-shell"),name=decodeURIComponent(element.dataset.municipality||""),fieldId=decodeURIComponent(element.dataset.field||"");body.innerHTML='<div class="akn-status">Laddar ÅkerNorm…</div>';aknLoadMunicipality(name).then(data=>{if(!element.isConnected)return;body.innerHTML=`<div class="akn-intro">Normal produktionsnivå – inte prognos för nästa skördeår. Grödor visas från mest till minst tillgängligt underlag.</div>${aknHelp()}${aknRenderField(data,fieldId)}`;element.dataset.loaded="1"}).catch(error=>{console.error(error);body.innerHTML='<div class="akn-status">ÅkerNorm kunde inte laddas.</div><button class="akn-retry" type="button" onclick="this.closest(\'details\').dataset.loaded=\'\';akernormToggle(this.closest(\'details\'))">Försök igen</button>'})}
...
def patch_html(html: str, mapping: dict[str, str]) -> str:
    ...
    required = (
        ...
        "Begrepp och gula markeringar", "Skiftesjustering", "Årsberoende grödkod", "Årsvisa grödnamn",
        "Högre osäkerhet", "Svag skiftesjustering", "Flera grödor", "0,05 t/ha",
