#!/usr/bin/env python3
"""STOPPUNKT C1: bounded 10 m Sentinel-2 rasters for independent 1000-field C pilot."""
...
CFG=ROOT/"config"/"akerpuls_prelim_fields_2026_c1.json"
...
def main():
    ...
    pdir=Path(args.pilot_dir or cfg["c0_selection_dir"])
    ...
    if len(g)!=expected:
        raise RuntimeError(f"Expected C0 pilot of {expected} fields, got {len(g)}")
    if "parent_field_id_2025" not in g.columns:
        raise RuntimeError("C0 pilot lacks parent_field_id_2025")

    # Freeze/contract consistency guard: C1 must use exactly the B1 preprocessing semantics.
    if cfg["preprocessing_contract"]!="IDENTICAL_TO_B1":
        raise RuntimeError("C1 preprocessing contract must be IDENTICAL_TO_B1")
    if cfg["source_bands"]!=b1.SOURCE_BANDS:
        raise RuntimeError("C1 source bands differ from B1")
    if cfg["snapshot_bands"]!=b1.SNAPSHOT_BANDS:
        raise RuntimeError("C1 snapshot bands differ from B1")
    ...
    # Scale a planning estimate from the actually measured B1 cost, if available.
    ...
    if ref_manifest.exists():
        ...
        rw=int(ref["grid"]["width"]); rh=int(ref["grid"]["height"])
    ...
    for snap,days in master["snapshots"].items():
        ...
        for day in days:
            ...
            request_rows.append({
                "snapshot":snap,"date":day,"cache_hit":hit,
                "reported_pu":meta.get("reported_pu"),"bytes":meta["bytes"]
            })
            ...
            if transform is None:
                ...
                raise RuntimeError("C1 source raster grid mismatch")
    ...
    labels=rasterize(
        ...
        out_shape=(height,width),transform=transform,fill=0,dtype="int32",all_touched=False
    )
    ...
    manifest={
      "schema_version":"akerpuls-prelim-fields-2026-c1-raster-v1",
      "status":"PASS",
      ...
      "next_step":"C2 run B2-style candidate discovery, then apply the locked split rule without tuning."
    }
    (out/"c1_manifest.json").write_text(json.dumps(manifest,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
